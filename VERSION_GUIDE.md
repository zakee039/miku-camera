...
| 名称 | 当前值 | 说明 |
|------|--------|------|
| **versionName** | `1.1` | 用户可见版本号（关于页、商店、APK 信息） |
| **versionCode** | `2` | 整数，每次上架/发版必须 **严格递增** |
| **Git tag** | `1.1` | 与 versionName 一致（本仓库不用 `v` 前缀） |
| **VERSION 文件** | `1.1` | 给人与脚本一眼看到的项目版本 |

> 历史：`0.1.0` 为初版快照；`1.1` 为所见即所得布局与交互大版本。
...
```kotlin
defaultConfig {
    versionCode = 2          // 整数，每次发版 +1（不可回退）
    versionName = "1.1"      // 与用户沟通的版本号，与 tag 一致
}
```
