...
| 名称 | 当前值 | 说明 |
|------|--------|------|
| **versionName** | `1.4.3` | 用户可见版本号（关于页、商店、APK 信息） |
| **versionCode** | `11` | 整数，每次上架/发版必须 **严格递增** |
| **Git tag** | `1.4.3` | 与 versionName 一致（本仓库不用 `v` 前缀） |
| **VERSION 文件** | `1.4.3` | 给人与脚本一眼看到的项目版本 |

> 历史：`0.1.0` 初版；`1.1` 所见即所得布局；`1.2` 定位/门牌/隐私说明与产品化完善；`1.2.1` 记住上次选择的水印；`1.2.2` 修复前置摄像头横屏方向和镜像问题；`1.3.0` 更名为 miku camera 并完善水印编辑与布局；`1.3.1` 修复水印名称持久化；`1.4.0` 新增 AI mode、可配置 AI 服务与风格选择；`1.4.1` 新增内置 miku 水印并调整 AI 按钮；`1.4.2` 完善 AI 水印控制、Miku 专属品牌、作者信息与关于页面；`1.4.3` 拆分图像参数与多组 API 配置，并加入服务商预设。
