...
| 名称 | 当前值 | 说明 |
|------|--------|------|
| **versionName** | `1.6.5` | 用户可见版本号（关于页、商店、APK 信息） |
| **versionCode** | `18` | 整数，每次上架/发版必须 **严格递增** |
| **Git tag** | `1.6.5` | 与 versionName 一致（本仓库不用 `v` 前缀） |
| **VERSION 文件** | `1.6.5` | 给人与脚本一眼看到的项目版本 |

> 历史：`0.1.0` 初版；`1.1` 所见即所得布局；`1.2` 定位/门牌/隐私说明与产品化完善；`1.2.1` 记住上次选择的水印；`1.2.2` 修复前置摄像头横屏方向和镜像问题；`1.3.0` 更名为 miku camera 并完善水印编辑与布局；`1.3.1` 修复水印名称持久化；`1.4.0` 新增 AI mode、可配置 AI 服务与风格选择；`1.4.1` 新增内置 miku 水印并调整 AI 按钮；`1.4.2` 完善 AI 水印控制、Miku 专属品牌、作者信息与关于页面；`1.4.3` 拆分图像参数与多组 API 配置，并加入服务商预设；`1.5.0` 接入火山方舟/Qwen/Gemini 多协议、列表 UI 改造、提示词解耦，并新增 AI 会话恢复（进程被杀后保留原图与提示词可恢复界面）；`1.5.1` AI 图片保存后保留结果页面，并提供重新拍摄与修改提示词操作；`1.6.1` 新增 AI 事务后台处理、悬浮窗、连续权限申请及 AI 照片导入；`1.6.2` 优化 AI 照片导入入口的可见性与布局；`1.6.3` 完善 AI 事务交互与重试流程；`1.6.4` 保存完整事务配置、隔离任务原图、同步重试状态并优化事务结束返回性能；`1.6.5` 关联 AI/原图文件名并支持拖动事务悬浮窗。
...
```kotlin
defaultConfig {
    versionCode = 11         // 整数，每次发版 +1（不可回退）
    versionName = "1.4.3"    // 与用户沟通的版本号，与 tag 一致
}
```
