...
| 名称 | 当前值 | 说明 |
|------|--------|------|
| **versionName** | `1.2` | 用户可见版本号（关于页、商店、APK 信息） |
| **versionCode** | `3` | 整数，每次上架/发版必须 **严格递增** |
| **Git tag** | `1.2` | 与 versionName 一致（本仓库不用 `v` 前缀） |
| **VERSION 文件** | `1.2` | 给人与脚本一眼看到的项目版本 |

> 历史：`0.1.0` 初版；`1.1` 所见即所得布局；`1.2` 定位/门牌/隐私说明与产品化完善。
...
```kotlin
defaultConfig {
    versionCode = 3          // 整数，每次发版 +1（不可回退）
    versionName = "1.2"      // 与用户沟通的版本号，与 tag 一致
}
```
